...
(defn execute-map
  [ds sql-map]
  (let [sql-string (sql/format sql-map)] (jdbc/execute! ds sql-string)))

(def ep execute-map)
(def ds (jdbc/get-datasource dev-db))

(def default-actions
  {:quick-attack   {:id   :quick-attack
                    :name "Quick Attack"}
   :hard-attack    {:id   :hard-attack
                    :name "Hard Attack"}
   :precise-attack {:id   :precise-attack
                    :name "Precise Attack"}
   :cast-spell     {:id   :cast-spell
                    :name "Cast Spell"}
   :quick-defend   {:id   :quick-defend
                    :name "Quick Defend"}
   :hard-defend    {:id   :hard-defend
                    :name "Hard Defend"}
   :precise-defend {:id   :precise-defend
                    :name "Precise Defend"}
   :focus          {:id   :focus
                    :name "Focus"}
   :full           {:id   :full
...
(defn generate-default-actions
  ...
  (for [[action action-map] default-actions]
    (ep ds
        (sd.mutations/generate-action-type {:action-type-inputs
                                            {:name (:name action-map)
...
(comment
 (graphql/graphql-query {:operation {:operation/type :mutation
             ...
             (graphql/graphql-query
              {:operation {:operation/type :mutation
               ...
               :variables [{:variable/name :$campaign_id
                            :variable/type :String!}]
               :queries   [[:create_session {:campaign_id :$campaign_id}
                            [:id [:campaign [:id]]]]]})
  ...
  (l/execute schema
             (graphql/graphql-query
              {:queries [[:list_campaigns
                          [:id :number :state
                           [:dungeon_master [:id :username :email]]]]]})
             nil
             {:sky-deck/datasource ds
              :sky-deck/auth       {:person-id ivan-id}}))


(comment (sky-deck.queries/person-by-username ds "ivan")
         (sky-deck.queries/campaign-by-number ds 1)
         (sd.graphql/compile-dungeon-master-api)
         (create-campaign ds {:name "test"})
         (create-session
          ds
          "Q2FtcGFpZ246NzAwYzhhMjYtMmU2ZC0xMWVhLThhYTMtOTc5YzhlYjllMThi")
         (list-campaigns ds))
...
(comment
 ...
 (ep ds
     (sd.mutations/generate-person {:new-id        ivan-id
                                    :person-inputs {:email "iwillig@gmail.com"
                                                    :username "ivan"
                                                    :password "password"}}))
 (ep ds
     (sd.mutations/generate-person {:new-id        eric-id
                                    :person-inputs {:email    "eric@example.com"
                                                    :username "eric"
                                                    :password "password"}}))
 (ep ds
     (sd.mutations/generate-campaign {:new-id          current-campaign-id
                                      :campaign-inputs {:dungeon_master_id
                                                        eric-id}}))
 (ep ds
     (sd.mutations/generate-character
      {:person-id        #uuid "dd91ac10-2d83-11ea-abec-abc86e648df9"
...
(comment (sql/format (sd.mutations/generate-person
                      {:new-id        (sd.mutations/new-id)
                       :person-inputs {:email    "iwillig@gmail.com"
                                       :username "ivan"
                                       :password "password"}}))
         (sd.mutations/generate-campaign {:campaign-inputs {}})
         (sql/format (sd.mutations/generate-action-type
                      {:action-type-inputs {:name "Hard Attack"
                                            :slug "hard-attack"}})))
