...
(defn init-test-system!
  ...
  (let [config (sd.config/new-system :test)
        ;; Remove http server in test mode to prevent yada from starting a real
        ;; http server
...
(defn gen-default-tx-data
  ...
  (let [person-id (UUID/randomUUID)
        person (sd.mutations/generate-person
                {:new-id        person-id
                 :person-inputs (assoc (gen/generate
                                        (s/gen ::sd.mutations/person-inputs))
                                       :password
                                       "password")})
        ...
        (sd.mutations/generate-character
         {:person-id        person-id
          :new-id           character-id
          :character-inputs (gen/generate (s/gen
                                           ::sd.mutations/character-inputs))})
        campaign-id (UUID/randomUUID)
        default-campaign (sd.mutations/generate-campaign
                          {:new-id          campaign-id
                           :campaign-inputs {:dungeon_master_id person-id}})
  ...
  (with-system-fn
   (fn [system]
     (let [init-objects (init-db system)
           ...
           battle-session (graphql-request
                           {::system    system
